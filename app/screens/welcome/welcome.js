...
export default function WelcomeScreen() {
		...
		const unsub = onSnapshot(q, async snap => {
			try {
				const data = snap.docs.map(d => {
					...
					const img = d.data().ImagenURL || '';
					const descripcion = d.data().Descripcion || '';
					return {
						id: d.id,
						title: nombre || 'Bienvenido',
						subtitle: d.data().Subtitulo || 'La aventura comienza aquí',
	...
	const next = () => {
		...
		setNextReady(false);
		if (targetSlide?.imageURL && !preloaded.has(targetSlide.imageURL)) {
			Image.prefetch(targetSlide.imageURL).finally(() => {});
		}
	};
	...
	const currentOpacity = pendingIndex != null ? progress.interpolate({ inputRange:[0,1], outputRange:[1,0] }) : 1;
	const nextOpacity = pendingIndex != null ? progress : 0;
	const currentTextTranslate = pendingIndex != null ? textCross.interpolate({ inputRange:[0,1], outputRange:[0,-18] }) : 0;
	const nextTextTranslate = pendingIndex != null ? textCross.interpolate({ inputRange:[0,1], outputRange:[18,0] }) : 0;
	const currentTextOpacity = pendingIndex != null ? textCross.interpolate({ inputRange:[0,0.5,1], outputRange:[1,0,0] }) : 1;
	const nextTextOpacity = pendingIndex != null ? textCross.interpolate({ inputRange:[0,0.5,1], outputRange:[0,1,1] }) : 0;
			...
			{!loading && slides.length > 0 && (
				<>
					<View style={{ flex:1 }}>
						...
						{pendingIndex != null && slides[pendingIndex] && (
							<Animated.View style={[styles.absoluteFill,{ opacity: nextOpacity },styles.layerBase]} pointerEvents="none">
								<ImageBackground
									source={slides[pendingIndex].imageURL ? { uri: slides[pendingIndex].imageURL } : require('../../assets/images/imagen_de_prueba.jpg')}
									style={styles.bg}
									resizeMode="cover"
									imageStyle={styles.imageFill}
									fadeDuration={0}
									onLoadEnd={() => { loadedMap[slides[pendingIndex].id] = true; setNextReady(true); if (!animating) runTransition(pendingIndex); }}
								>
									<View style={styles.overlay} />
									<View style={styles.topRow}>
										<TouchableOpacity onPress={finish} hitSlop={{ top:10, bottom:10, left:10, right:10 }}>
									...
									<Animated.View style={[styles.textBlock, { transform:[{ translateY: nextTextTranslate }] }]}> 
										<AnimatedThemedText variant="title" style={[styles.title, styles.layerText, { opacity: nextTextOpacity }]}>{slides[pendingIndex].title}</AnimatedThemedText>
										<AnimatedThemedText variant="subtitle" style={[styles.subtitle, styles.layerText, { opacity: nextTextOpacity }]}>{slides[pendingIndex].subtitle}</AnimatedThemedText>
										<AnimatedThemedText variant="body" style={[styles.description, styles.layerText, { opacity: nextTextOpacity }]}>{slides[pendingIndex].description}</AnimatedThemedText>
									</Animated.View>
								</ImageBackground>
							</Animated.View>
						)}
					...
					<View style={styles.footer}>
						<View style={styles.dotsRow}>
							{slides.map((s, i) => {
								const active = i === (pendingIndex != null ? pendingIndex : index);
