...
export default function CascadasScreen(){
	...
	useEffect(() => {
		...
		const tryServerQuery = async () => {
			try {
				const q1 = query(col, where('Categoria', '==', 'Cascada'));
				...
				const q2 = query(col, where('categoria', '==', 'Cascada'));
				...
				console.error('Query error cascadas:', e);
			}
			unsub = onSnapshot(col, (snap) => {
				try {
					...
					}).filter(p => (p.categoria || '').toLowerCase().includes('cascad') || (p.categoria || '').toLowerCase().includes('poza'));
					setPlaces(mapped);
				} catch (e) { console.error('Lugares snapshot error (cascadas fallback):', e); }
			}, (err) => console.error('Snapshot cascadas error:', err));
		};
		tryServerQuery();
		...
		<View style={{ flex: 1, backgroundColor: '#f5f7fb' }}>
			<View style={styles.headerOrange}>
				...
				<View style={{ flex: 1, paddingLeft: 16 }}>
					<Text style={styles.headerTitle}>Cascadas</Text>
					<Text style={styles.headerSub}>Explora las cascadas y pozas naturales</Text>
				</View>
			</View>
